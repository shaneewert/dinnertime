...
export default function SearchableRecipeList({ initialRecipes, onRecipeClick, xstyle }) {
  ...
  return (
    <div>
      ...
      {renderedRecipes.map((recipe) => (
        <RecipeRow xstyle={xstyle} key={recipe.id} recipe={recipe} onClick={onRecipeClick} />
      ))}
    </div>
  );
}
