...
export default function SearchableRecipeList({ initialRecipes, onRecipeClick, onRecipeDeleted }) {
  ...
  return (
    <div>
      ...
      {renderedRecipes.map((recipe) => (
        <RecipeRow key={recipe.id} recipe={recipe} onClick={onRecipeClick} onRecipeDeleted={onRecipeDeleted} />
      ))}
    </div>
  );
}
